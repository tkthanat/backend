...
# (ค้นหากล้องแบบไม่ทดสอบเฟรม เพื่อให้บูตเร็วขึ้น)
discovered_devices = discover_local_devices(test_frame=False)
# (กรองเอาเฉพาะอันที่ 'opened' ได้)
...
if len(available_sources) > 0:
    CAMERA_SOURCES['entrance'] = available_sources[0]  # entrance = กล้องตัวแรก
else:
    CAMERA_SOURCES['entrance'] = "0"  # ถ้าไม่เจอเลย ให้ใช้ "0" เป็นค่าเริ่มต้น

if len(available_sources) > 1:
    CAMERA_SOURCES['exit'] = available_sources[1]  # exit = กล้องตัวที่สอง
else:
    CAMERA_SOURCES['exit'] = CAMERA_SOURCES['entrance']  # exit = ใช้กล้องเดียวกับ entrance
